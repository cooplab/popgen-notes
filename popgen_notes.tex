...
to whether $\left(\prod_{i=t}^{\tau} \frac{w_{1,i}}{w_{2,i}}  \right)$ is
...
\end{equation}
The term $\sqrt[t]{\prod_{i=t}^{\tau}w_{1,i}}$ is the geometric mean fitness of allele
...
\begin{equation}
\Delta p_t = \frac{p_tq_t (p_ths + qs(1-h))}{\wbar},
\label{deltap_direct}
\end{equation}
where
\begin{equation}
...
\begin{equation}
	\Delta p_t = \frac{p_tq_t s/2}{\wbar}.
	\label{deltap_add}
\end{equation}
...
relative fitness in environment A  & $w_{11,B}$ & $<w_{12,B}<$ & $w_{22,B}$ \\
...
completely recessive ($h>0$), so that the heterozygotes suffer some disadvantage, too. We denote by $\mu = \mu_{12}$ the  mutation rate per generation from $A_1$ to the deleterious allele $A_2$, and assume that there is no reverse mutation ($\mu_{21} = 0$).  Let us assume that selection against $A_2$ is relatively strong compared to the mutation rate, so that it is justified to assume that $A_2$ is always rare, i.e.\ $q_t \ll 1$. Compared to previous sections, for mathematical clarity, we also switch from following the frequency $p_t$ of $A_1$ to following the frequency $q_t$ of $A_2$. Of course, this is without loss of generality.} We are interested in the case where  allele $2$ is rare within
