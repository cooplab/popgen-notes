...
individuals with covariance matrix $V_A A$ where the the entries of
$A_{ij}=F_{i,j}$. Given the matrix $A$ we can estimate $V_A$.
...
In a similar way to before we can estimate these quantities by
studying the covariance in different traits between relatives: 
\begin{equation}
Cov(X_{1,i},X_{2,j}) = F_{i,j} V){A,1,2}
\end{equation}
