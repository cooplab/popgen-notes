...
\begin{equation}
F_{IS} =1-\frac{H_T}{H_S}= 1-\frac{f_{12}}{2p_Sq_S} \label{eqn:FIS}
\end{equation}
...
total population. Well as a simple extension of this we could imagine
comparing the expected heterozygosity in the subpopulation ($H_S$) to
the expected our total population ($H_T$), via $F_{ST}$
\begin{equation}
...
\end{equation}
using the fact that $p^2+2pq+q^2=1$. Well the form of this
...
We can use our understanding the sharing of alleles between relatives
...
allele $1$ a this locus increasing your trait value by $a_l$.
The phenotype of an individual, let's call her $i$, is $Y_i$.
Her genotype of an individual, at SNP $l$, is
...
\begin{equation}
X_{A,i} = \E (X_i | G_{i,1},\cdots,G_{i,L}) =  \sum_{l=1}^L G_{i,l} a_{l}
\end{equation}
Now in reality the genetic phenotype is a function of the
...
\begin{equation}
X_i =   X_{A,i} + X_{E,i} \label{pheno_geno_environ}
\end{equation}
...
$X_{A,i}$ by a normal distribution due to the central limit theory (see R exercise). Thus if we can
...
\end{equation}
in doing writing this we are assuming that there is no covariance between $X_{G,i}$
...
case $X_{P1}=X_{M1}$ and so $Cov(X_{P1},X_{M2})=Var(X_{P1})$ and all
the other covariances in eqn. \ref{cov_rels_1} zero, and half
the time she transmits her maternal allele to the child
$Cov(X_{M1},X_{M2})=Var(X_{P1})$ and all the other terms zero. By this
...
share IBD. Consider a pair of relatives ($x$ and $y$) with a probability $r_0$,
...
$Cov((X_{1M}+X_{1P}),(X_{2M}+X_{2P}))=\half
Var(X_{1M})=\frac{1}{4}V_A$, and when they share two alleles $Cov((X_{1M}+X_{1P}),(X_{2M}+X_{2P}))=
Var(X_{1M})=\half V_A$. Therefore, the general covariance between two
relatives is
\begin{equation}
Cov(X_1,X_2) = r_0 \times 0 + r_1 \frac{1}{4}V_A + r_2 \half V_A =
F_{x,y} V_A
\end{equation}\\
...
fraction of the total phenotypic variance ($S/V$).\\
...
association between loci. We'll define the recombination fraction ($r$) to
...
alleles at different loci. Our two loci, which segregate alleles $A/a$ and $B/b$, have a allele
...
We've now think about what happens to LD over the generations if we
...
The numbers individuals carrying allele 1 and allele 2 in generation
$t$ are $P_t$ and $Q_t$
respectively. The current frequency of allele 1 is $p=P_t/(P_t+Q_t)$.\\

In the next generation the number of of type 1 and 2 individual is
$P_{t+1} = w_1 P_t$ and $Q_{t+1}=w_2 Q_t$
The mean fitness of our population is  $w_1 p_t+w_2 q_t$, i.e. the
...
\end{equation}
The change in frequency form one generation to the next is
\begin{equation}
...
\begin{equation}
\Delta_S q = \frac{pq(\wbar_2 - \wbar_1)}{\wbar}  \approx 2 hs q
\end{equation}
...
\end{equation}
To obtain
$q(x+\Delta x,t)$ lets take a taylor series expansion of $p(x,t)$
\begin{equation}
