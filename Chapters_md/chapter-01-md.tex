...
\caption{ {\it Drosophila melanogaster} holds a special place in the
  ...
  on natural genetic variation.  \BHLNC{Contributions to the genetics
    of {\it Drosophila melanogaster}
...
\begin{table*}
  \begin{center}
  \small
\setlength{\tabcolsep}{.55 \tabcolsep}   % https://tex.stackexchange.com/questions/307770/centered-tabular-column-with-narrow-columns
...
where the first bracketed term gives the pairwise differences between a and
...
\sidenote{Technically we would need to divide by the total number of possible
  point mutations that would result in a synonymous change; this is because
some mutational changes at a particular nucleotide will result in a
non-synonymous or synonymous change depending on the base-pair change.}
...
\marginnote{Throughout this chapter we'll be making use of the basic rules of
probability to find the probabilities of combinations of events, e.g. the
alleles found in an individual, see Appendix \ref{Section_rules_prob} for a
refresher.}
...
\caption{Cauliflower mushrooms ({\it Sparassis crispa}) parasitize tree roots and form these
  ...
  apparently taste like noodles. In a collection of 18 fruiting
  bodies from a {\it Sparassis} population, all individuals were heterozygotes for mating type and 17 different mating types were genetically
...
We can write out terms like \eqn \eqref{eqn:IBD_relly_calc} for all of
...
\label{eqn:Fhat}
\end{equation}
\marginnote{The hat on the F here denotes that this is an estimate
  from a sample.}
...
% \begin{question}{}
% Charles the 2$^{nd}$ of Spain was the offspring of an uncle-niece marriage. 
% In answering this question, ignore the other deeper inbreeding loops in his pedigree.\\
