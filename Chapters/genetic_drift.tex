...
\gc{HUDSON}, but we won't give this here. The general form is: 
\begin{equation}
...
\paragraph{Demography and the site frequency spectrum}
...
For example, if the population has grown rapidly then the pairwise-coalescent
...
census size. 
The second consequence is that the deeper coalescent branches are
...
As an example of the osberved frequency spectrum from a very large
sample of humans is shown in Figure \ref{fig:Human_growth}. For
comparison the neutral frequency spectrum, eqn
\eqrefl{eqn:neutral_freq_spec, is shown as a red line. There are
  vastly more rare alleles than expected under our neutral constant
  population size coalecent model. 
...
\caption{Data from 202 genes from 14002 people of European ancestry (28004 alleles). Note
  the double log-scale. } \label{fig:Human_growth}
\end{marginfigure}
...
mutaiton and also does not account for multiple hits. Therefore, in
practice $d_N/d_S$ is more usually calculated by model-based
likelihood and bayesian methods
that can account for these features (see \gc{XXXX}). 


If we are willing to make the assumption that all synymous changes are
neutral $d_S=2T \mu$. (Note that synonymous changes can sometimes be subject to
...
alleles is section \gc{XX}).  If $T$ generations of divergence have
elapsed between the two populations 
\begin{equation}
2T (1-C - B) \mu  + 2T B f_B \mu
\end{equation}

We often see that $d_N/d_S \ll 1$, this is consistent with the view
...
\begin{marginfigure}
\begin{center}
\includegraphics[width=\textwidth]{illustration_images/Genetic_drift/Colobus/19792029373_fcce706e67_k.jpg}
\end{center}
...
\begin{marginfigure}
\begin{center}
\includegraphics[width=\textwidth]{illustration_images/Genetic_drift/Hoatzin/14747388314_85798ba97e_z.jpg}
\end{center}
