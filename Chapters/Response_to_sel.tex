...
like this are called `realized heritability'. 
\begin{question}
Estimates of the `realized heritability' can be produced in artificial selection experiments, see for example Figure \ref{Illinois_LTS_breeders_eq}.
The mean oil content in 1897 was $4.78$, among the $24$ individuals
chosen to breed to for the next generation the mean $5.2$. The
...
i.e. our population will keep up a linear response to selection.
 ...
 \caption{Density plots showing the phenotypic distributions of the
   up- and down-selection populations over time. Data available
...
decades on the island of Daphne Major and seen that
...
\section{Alternative formulations of the Breeder's equation.}
So directional evolution occurs as selection drives a change in
the mean phenotype within a generation. But how does this relate to
the fitness of our organisms? To think more carefully about this change within a
...
\begin{equation}
\wbar =  \int_{-\infty}^{\infty} p(x) w(x) dx.
\end{equation}
...
One common metaphor when talk about evolution is that of a population exploring an adaptive landscape with natural selection pushing a population
