...
\paragraph{Fitness landscapes}
 ...
 \end{center}
 \caption{ } \label{fig:fitness_landscape_1D_w_wbar}  
 \end{figure*}
...
landscapes. \citeauthor{lande1976natural} showed that the response to
...
\marginnote{ This follows from the fact that $\nicefrac{\partial \log
    \wbar}{\partial \bar{x}} = \nicefrac{1}{\wbar} \nicefrac{\partial
    \wbar}{\partial \bar{x}}$. We can then move the derivative inside the integral
\begin{align}
 \nicefrac{\partial \wbar}{\partial \bar{x}} &= \int_{-\infty}^{\infty}w(x)
                               \nicefrac{\partial p(x)}{\partial \bar{x}}  dx \nonumber\\
...
marching up the nearest hill of mean fitness, and may become trapped
on a local peak (Figure \ref{fig:fitness_landscape_1D} ). 

%but this isn't necessarily the highest fitness peak it's just
%which ever one it started climbing.
...
selection, which act on the variance.
...
\end{center}
\caption{ \gitcode{https://github.com/cooplab/popgen-notes/blob/master/Journal_figs/Quant_gen/birth_weight/birth_weight_selection.R} } \label{figBirth_weight}  
\end{marginfigure}

Under stablizing selection the individuals with extreme phenotypes in
either tail have lower fitness. A classic case of stablizing selection
is birth weight in humans \citep{karn1951birth}. Mary Karn collected
data from nearly fourteen thousand pregnancies, recording the birth
weight and mortality. 
 
 goldenrod gall fly ({\it Eurosta solidaginis}), aka the goldenrod
 ball gallmaker (which is pretty great name).
