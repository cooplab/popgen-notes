...
\end{center}
\caption{ } \label{fig:Geospiza_fortis}  
\end{marginfigure}
...
\end{center}
\caption{ } \label{fig:Darwins_Finches_unpred}  
\end{figure}
...
$w(X)$ ($S = Cov \left(X, \nicefrac{w(X)}{\wbar} \right)$). Thus our change in mean phenotype is directly a measure of the
...
The phenotype-fitness covariance divided by the phenotypic variance, $Cov(X, \nicefrac{w(X)}{\wbar} )/ V$, is the slope of the linear regression of phenotype on fitness. Let's call this slope the fitness gradient and denote it by $\beta$. Then, equivalently, we can write the breeder's equation as
\begin{equation}
R= V_A \beta
\end{equation}
...
\begin{equation}
  R = \frac{V_A}{\wbar} \frac{\partial \wbar}{\partial \bar{z}} = V_A
  \frac{\partial \log \left(\wbar \right)}{\partial \bar{z}}
\end{equation}

\marginnote{ This follows from the fact that $\nicefrac{d \log
    \wbar}{d \bar{x}} = \nicefrac{1}{\wbar} \nicefrac{d \wbar}{d
\bar{x}}$. We can then move the derivative inside the integral
\begin{align}
 \nicefrac{dp(x)}{d \bar{x}} &= \int_{-\infty}^{\infty}w(x)
                               \nicefrac{d p(x)}{d \bar{x}}  dx \nonumber\\
 =\int_{-\infty}^{\infty}w(x)     \frac{(x-\bar{x})}{V}  dx
                             & = \nicefrac{cov(w(x),x) }{var(x)} \nonumber\\
\end{align}
...
\end{center}
\caption{ Photo: Peter J. Park} \label{fig: Stickleback_fossil_traj}  
\end{marginfigure}
...
\end{center}
\caption{Fossil measurements are in 250 year bins. } \label{fig: Stickleback_fossil_traj}  
\end{marginfigure}

\citet{bell2006inferring} provided a remarkable time-series of
stickleback evolution from a fossil lake-bed in Nevada. In this lake
...
heavily armoured stickleback species. In Figure \ref{fig:
  Stickleback_fossil_traj} the population mean number of
...
its new value, presumably an fitness optimum in their new environment, and the long time subsequent time interval over which
...
fitness landscape to these time-series data. They fitted a model with
a single peak of mean fitness. Their fitted fitness
...
fluctuations represent genetic drift, with selection acting to restore
...
goldenrod gall fly ({\it Eurosta solidaginis}), aka goldenrod ball
gallmaker, 
...
\caption{Australian Honey-pot Ant ({\it Camponotus inflatus}). Honey ants are gorged with
  ...
  social Hymenoptera (1897) Lubbock, J.}{https://www.biodiversitylibrary.org/page/9657360\#page/485/mode/1up}{Smithsonian Libraries}  } \label{fig:turkey}
\end{marginfigure} 
