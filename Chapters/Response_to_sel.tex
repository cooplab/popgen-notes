...
benefit of others. That selection favours such apparent acts of altruism is puzzling at first sight. \citet{hamilton1964genetical,hamilton1964genetical2} supplied the first general evolutionary explanation of such altruism. \marginnote{\citet{smith1964kin} coined the name kin selection to describe Hamilton's approach to this problem. It's also sometimes called the inclusive fitness approach, as we need to include not just one individual's fitness but the weighted sum of all the fitness of all their relatives. }
...
\begin{equation}
 2F B > C 
\end{equation}
where $F$ is the average kinship coefficient between the interacting individuals ($i$ and $j$), here $2F$ is the average number of alleles that
our pairs of individuals share identical by descent at a locus. So the altruistic behaviour will spread even if it is costly to the individual if its cost is paid off by the benefit to sufficiently related individuals. 

\marginnote{ Here we'll follow a simplified version of \citet{queller1992quantitative}'s treatment, to re-derive Hamilton's rule in a quantitative genetics framework (Hamilton's original work did this in a population genetics framework).}
Where does this result come from? Well, we can use our quantitative genetics framework to gain some 
intuition by deriving a simple version of Hamilton's Rule by thinking about the phenotypes of an individual's kin as genetically correlated phenotypes. 
...
To sketch a proof of this result, let's assume that our focal $i$ individual's relative \erin{is this not absolute fitness?} fitness can be written as 
\begin{equation}
W(i,j)= W_0 + W_i +W_j
\end{equation}
...
\begin{eqnarray}
  ...
  2 F_{i,j} B > C 
\end{eqnarray}
Seen from this perspective, \citeauthor{hamilton1964genetical}'s rule is simply a statement that altruistic behaviours can spread via kin-selection, if the the average cost to an altruistic individual is paid back on average through the benefits of interacting with altruistic relatives (kin). \erin{this phrasing is a little misleading because it's 'paid back' to the allele, but not to the individual. I'd suggest something like 'if the fitness cost of altruism to an allele is paid back on average through the benefits of interacting with altruistic relatives (kin).}
...
The selection for longer tails in males could come about because longer tails are correlated with higher male fitness
