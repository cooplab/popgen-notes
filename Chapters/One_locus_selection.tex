...
\end{center}
\caption{For the three Soay sheep genotypes: the offspring per year  ({\bf left}), the probability of
...
\ec{this trade off between fitness at different life history stages}.
...
\bar{w}_2$, is there no additive genetic variance  ($V_A = 0$), as the linear regression \ec{of fitness on genotype} is zero. 


\paragraph{Underdominance.} Another case \ec{of balancing selection} that is of potential interest is the case of fitness
...
0$ and selection is reasonably strong), they are of interest in the study of speciation and hybrid zones. That is because alleles $A_1$ and $A_2$ may have arisen in a stepwise fashion, i.e.\ not by a single mutation, \ec{but} in separate subpopulations. In this case, heterozygote disadvantage will play a potential role in species maintenance, if isolation of the subpopulations is not complete.\\
...
lifespan of {\it Drosophila} we can see seasonally-varying selection. The first example is an inversion
allele in {\it Drosophila pseudoobscura} populations. Throughout western North America, two orientations of the
chromosome, two \ec{'inversion alleles'}, exist: the Chiricahua and Standard
alleles. \citeauthor{wright:46}
\graham{need earlier Dob. ref IX} investigated the frequency of these inversion alleles over
...
frequency of this allele over time and found it \ec{oscillated with the seasons}. She and her coauthors also determined that these alleles had large effects on traits such as developmental time and fecundity, which could mediate
the maintenance of this polymorphism through life-history trade-offs.
...
\end{equation}
To obtain this equation, we have ignored the $p_{t}^2$ term \ec{(because it is very small when $p_t$ is small)} and we have assumed that $q_t \approx 1$ in the numerator.
...
\end{equation}
\erin{I think that this should just be t generations; or if it's t+1 generations later on top of the product shouldn't it be t not t-1? Same inconsistency affects the next 2 equations below} From this we can see, following our haploid argument from above, that the frequency of allele $A_1$ will increase when rare only if
\begin{equation}
...
(HINT: Let's write $w_{AA,\text{dry}}$ and $w_{AA,\text{wet}}$ for the fitnesses of the AA homozygote in the two environments. Then, if the
...
titled paper\cite{Wheeler:07}). This pink condition is thought to be due to
...
carries a unique set of these LOF alleles, usually in a heterozygotous state. We differ slightly in how
...
$1-hs$,  then an individual who's heterzoygote for two LOF mutations would have
...
heterozygotes and a relative fitness $0.2$. Assume that the
...
\end{question}
One consequence \ec{of segregating for low-frequency recessive deleterious alleles} is that inbred individuals from usually outbred
...
coefficient $F$, \erin{I changed from lower to upper case F in the equation below to match} deleterious alleles at each locus will find a
...
the allele now being expressed in homozygotes, and therefore exposed to selection, more often due to inbreeding.  All else being equal, then
