...
{\bf B)} Calculate the marginal fitnesses \erin{unclear if you want the marginal absolute or relative fitnesses} of the B and the b allele at the equilibrium frequency. 
\end{question}
...
As a toy example of this \ec{type of balanced polymorphism}, consider a plant population found in one of two different environments each
...
we can take the $t^{th}$ root to obtain the geometric mean fitness. Taking the $t^{th}$ root, \ec{we find} the geometric mean fitness of the AA allele  is $w_{AA,\text{dry}}^{\nicefrac{1}{2}}
...
\begin{question}
\erin{I feel like this Q belongs up right after Q9 before the fluctuating selection section} OPTIONAL trickier question.\\
...
Our two alleles were maintained in the population by heterozygote
advantage as the common allele had a disadvantage compared to the
other rarer allele.  Heterozygote advantage is just one example of
negative frequency-dependent selection, where the fitness of an allele
(or phenotype) decreases as it becomes more common in the population. 
While cases of long-term heterozygote advantage may be somewhat rare in nature,
negative frequency-dependent selection is likely a common form of
balancing selection.

One common mechanism that may create negative frequency-dependent
selection is predator-prey or pathogen-host dynamics, where
alleles conferring common phenotypes are at a disadvantage because 
predators or pathogens evolve or learn to \gc{XXX}
...
\times s$. That is, we have assumed that there is no interaction between mutation and selection.\erin{it's somewhat unclear what you mean by 'no interaction', my understanding is that we're saying that, for a single generation of selection, we don't need to think about selection against mutations arising in that same generation} This assumption is valid under our prior assumption that both $\mu$ and $s$ are small.\\
...
titled paper\cite{Wheeler:07}). \begin{marginfigure}
\begin{center}
...
What effect do such deleterious mutations at mutation--selection balance have on the population? It is common to \ec{quantify} the effect of deleterious alleles in terms of a reduction of the mean relative fitness of the population. For a single site at which a deleterious mutation is segregating at frequency $q_e = \mu/(hs)$, the population mean relative fitness is reduced to
\begin{equation}
...
How do these differences \ec{across individuals in total LOF mutations} mount up? \erin{I'm going to assume 'mount up' is British for 'add up'} Well, if we are willing to assume
that \ec{the fitness costs of deleterious alleles} interact multiplicatively, we can make some
...
\begin{question}
\gc{Assume that a deleterious allele has a relative fitness $.99$ in
heterozygotes and a relative fitness $0.2$ when present in the
homozygote state.} Assume that the
deleterious allele is at a frequency $10^{-3}$ at birth \erin{I'm not sure this is a problem per se, but I am only getting a difference in the 4th decimal place for A vs. B below due to the low frequency of this allele..w_bar in A is 0.9999792 and w_bar in B is 0.9997844} and the
genotype frequencies follow from HWE. \ec{Only considering the fitness effects of this locus, and measuring fitness relative to the most fit genotype, answer the following questions: }\\
...
One consequence of segregating for low-frequency recessive deleterious alleles is that \ec{inbreeding can reduce fitness}.  In typically outbred populations, the mean fitness of individuals
...
on mine tailings and in nearby meadows, a set of classic studies of \ec{population differences maintained by local adaptation to different soils}.
\begin{figure}
\begin{center}
...
adaptation. Zinc-intolerant alleles are \ec{nearly absent from the mine tailings because they prevent plants} from growing on \ec{these}
zinc-heavy soils; \ec{conversely, zinc-tolerant alleles do not spread into the meadow populations,  likely due to some trade-off or fitness cost of zinc-tolerance}.
...
To quickly sketch \ec{an equilibrium} solution to this \ec{scenario}, we'll \ec{take an approach} analogous
...
One example \ec{of locally adapted prey} are the rock pocket mice who live on the Pinacate
lava flow on the Arizona-Mexico border, studied by
\citet{hoekstra2004ecological}. These mice have much darker, more melanic pelts 
than \ec{the mice} who live on nearby rocky outcrops (see Figure \ref{fig:mig_sel_bal_mice}).   %https://hoekstra.oeb.harvard.edu/files/hoekstra/files/hoekstra2005vol.pdf 
...
Theodor Seuss Geisel}    We'll think of a \ec{bi-allelic} model where
...
zero in Figure \ref{fig:cline_main}). This flips to allele $2$ having the same advantage $s$ west of the transition (right of zero). 
...
$2$ decreases in a smooth cline. The width of this cline, i.e. the
...
be very rapid, \ec{and the cline narrow}, as we move across the environmental transition. In contrast, if individuals
...
\begin{equation}
  0.6 \sigma/\sqrt{s}
\end{equation}
miles. \erin{really, miles? lol very specific formula} The appendix below talks through the math underlying these
...
x)^2 g(\Delta x) d \Delta x = \sigma^2 $. \ec{All higher order terms in our Taylor series expansion cancel out} (as all high moments of the normal \ec{distribution} are zero). Looking at the change in allele frequency,
...
selection and migration terms together, we find \ec{the total change in allele frequency at location x in one generation is}\begin{equation} q(x,t+1) -
q(x,t) = s\gamma(x) q(x,t) \big( 1 - q(x,t) \big)+\frac{\sigma^2}{2}
\frac{d^2q(x,t)}{dx^2} \label{eqn:fisherKPP} \end{equation}In deriving this result, we
