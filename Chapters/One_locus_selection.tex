...
polymorphism in the population at an equilibrium frequency. What's
happening here is that the marginal fitness of the $1$ allele is
greater when it's at low frequency in the population ($\bar{w}_1 > \bar{w}_2 $)
...
of $A_1$ at which the mean fitness of the population is maximised.\\
...
\caption{For the three Soay sheep genotypes the offspring per year  ({\bf left}) and probability of
  surviving a year ({\bf middle}), the product of the two ({\bf
    right}). Thanks to Susan Johnston for supplying these numbers from \citet{Johnston}.
...
the rut.  A large-effect locus
controlling much of the genetic variation for horn size is polymorphic nearly the RXFP2 gene in this population, with two
alleles Ho$^p$ and Ho$^+$ segregating. The Ho$^+$ allele is associated with
...
effect on male, but female, fitness (see Figure \citep{fig:Soay_fitness}). The male Ho$^p$ homozygotes have lower yearly reproductive
success presumably due to the fact that they perform poorly in male-male
competition (left plot Figure \ref{fig:Soay_fitness}). Conversely the male Ho$^{+}$ homozygotes had lower
...
fitness  (right plot Figure \ref{fig:Soay_fitness}).  
