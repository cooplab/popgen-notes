...
As a toy example of this consider a plant population found in one of two different environments each
...
we can take the $t^{th}$ root to obtain the geometric mean fitness. Taking the $t^{th}$ root the geometric mean fitness of the AA allele  is $w_{AA,\text{dry}}^{\nicefrac{1}{2}}
...
the homozygotes, despite not being the fitest genotype in either
...
\times s$. That is, we have assumed that there is no interaction between mutation and selection. This assumption is valid given we assumed that both $\mu$ and $s$ are small.\\
...
heterozygote; it depends only on the mutation rate. Note that this applies only if the mutation is not totally recessive, i.e.\ if $h > 0$.
...
deleterious, some could even be advantageous, but the combined load of these
...
that these differences interact multiplicatively, we can make some
...
One consequence of segregating for low-frequency recessive deleterious alleles is that inbred individuals from usually outbred
...
is a common observation. This is wide-spread observation dates back to systematic
...
to selection, more often due to inbreeding.  All else being equal,
then populations with a high degree of inbreeding will purge their load. 
...
generations. Adaptation can potentially occur on much finer geographic scales
...
adaptation. Zinc-intolerant alleles are kept from growing on the
zinc-heavy soils of the mine, and zinc-tolerant alleles likely prevented from spreading to
the meadow by some tradeoff. 
...
selection model, to give the diploid
migration selection balance frequency as
\begin{equation}
q_{e,1} = \frac{m}{hs}
\end{equation}
...
cryptic against this dark substrate to better avoid visual predators. 
One example are the Rock pocket mice who live on the Pinacate
...
than those who live on nearby rocky outcrops (see Figure \ref{fig:mig_sel_bal_mice}).   %https://hoekstra.oeb.harvard.edu/files/hoekstra/files/hoekstra2005vol.pdf 
\citet{nachman2003genetic} determined that a dominant allele ($D$) at MC1R
is the primary determinant on this Melanic phenotype. The frequency of
this allele across study sites is shown in Figure
\ref{fig:mig_sel_bal_mice}. \citet{hoekstra2004ecological} found that
other, unlinked markers showed little differentiation over these populations,
suggesting that the migration rate is high. 

\begin{question}
\citet{hoekstra2004ecological} found that the dark $D$ allele was at
$3\%$ at the Tule Mountains study site. Using $F_{ST}$-based
approaches, for unlinked markers, they estimated that the per individual migration rate was $m=7.0 \times
10^{-4}$ per generation between this site and the  Pinacate lava
flow. What is the selection coefficient acting against the dark $D$ allele
at the Tule mountain site? 
\end{question}

\paragraph{The width of a genetic cline.}
We can also extend these ideas beyond our discrete model to a model of a population spread out
on a landscape where individuals migrate in a more continuous
fashion. To take a simple case of this let's assume a one dimensional
...
Theodor Seuss Geisel}    We'll think of a balletic model where
the homozygotes for allele 1 has an additive selective advantage $s$
over allele $2$ homozygotes to the east of our transition, left of
zero in Figure \ref{fig:cline_main}. This flips to allele $2$ having this advantage west of the
transition (right of zero). 
...
With this setup we get an get an equilibrium
...
geographic distance over which frequency change, depends on
...
be very rapid as we move across the environmental transition. While, if individuals
dispersal long distances, and selection is weak,  many alleles are being moved back and forth over the
...
\begin{equation}
  0.6 \sigma/\sqrt{s}
\end{equation}
miles. The appendix below talks through the math underlying these
