...
regression at each SNP in the genome. One common way to display \ec{the
...
\caption{The Manhattan plot zoomed in on the top-hit (red SNPs from Figure
  ...
  Figure 2 of \ec{needs end of sentence} } \label{fig:Apsen_zoom_Manhattan}
\end{figure}
...
has at this SNP. Her expected phenotype, given her genotype \ec{at all $L$ SNPs}, is then
\begin{equation}
...
\end{equation}
In writing \ec{the phenotypic variance as a sum of the additive and environmental contributions,} we are assuming that there is no covariance between $X_{G,i}$ and $X_{E,i}$ i.e. there is no covariance between genotype and environment. \\
...
Now we don't usually get to see the individual loci contributing to highly polygenic traits. Instead, we only get to see the distribution of the trait in the population. However, with the advent of GWAS in human genetics we can see some of \ec{the underlying genetics} using the many \ec{trait-associated} loci identified to date. Using the estimated effect sizes at each locus, each one of which is tiny, we can calculate the weighted sum over an individual's genotype as in equation \ref{pheno_geno}. This weighted sum is called the individual's polygenic score. To illustrate how polygenic scores work, we can take a set of 1700 SNPs, each chosen as the SNP with the strongest signal of association \ec{with height} in 1700 roughly independent bins spaced across the genome. Figure \ref{fig:Biobank_height_PGS} shows the distribution of a thousand individuals' polygenic scores calculated using these 1700 SNPs. The variance of these polygenic scores is $0.13$, the additive genetic variance calculated by equation \eqref{eqn:VA} is $0.135$, so the two are in good agreement.\erin{I feel like this needs interpretation - is this showing our low LD assumption is accurate? I don't really get it}
...
relatives. For example, if our phenotype was totally environmental \erin{I find this wording fits too well with the common misunderstanding of traits being environmental or genetic...would something like 'if the phenotypic differences between individuals in our population were solely determined by differences between the environments experienced by these different individuals,' } we
...
between the environment variation \erin{is it really environmental variation and genetic variation or just environment and genotype? } experience by an individual and the
...
the \ec{maternal and paternal} allelic effects in our individuals. We can use our results about
...
us in estimating $h^2$, \ec{because under models with non-additive effects, identical twins will have higher covariance than we'd expect just based on the alleles they share. This is because identical twins don't just share alleles, they share their entire genotypes, and thus 
...
\paragraph{Parent-midpoint offspring regression} 
...
\begin{question}
\ec{Briefly explain what Galton meant by 'regression towards
...
\end{equation}
assuming our loci are in linkage disequilibrium. Thus a genetic correlation arises due to pleiotropy, because loci that tend to affect trait 1 also systematically affect trait 2. For example, alleles associated with later Age at Menarche (AAM) in European females also tend to be positively associated with height (see Figure \ref{fig:AAM_height}), \ec{thereby creating} a genetic correlation between AAM and height. 
...
\end{center}
\caption{The additive effect sizes of loci associated with female Age at Menarche (AAM) and their effect size on Height in a European population. Data from \citet{pickrell2016detection}} \label{fig:AAM_height}   %\cite{potti:11} 
\end{marginfigure}
...
One  type of  genetic covariance we often think about is the covariance of male and female phenotypes. For example, below is the relationship between the forehead patch size for Pied fly-catcher fathers and their sons and daughters. The phenotype has been standardized to have \ec{mean $0$ } and variance $1$ \ec{in each group}. The phenotypic covariance of the sample of fathers and sons is $0.35$, while the phenotypic covariance of fathers and daughter is $0.23$. 
...
\caption{Relationship of standardized forehead patch size between
  fathers and sons and daughters in  Pied fly-catchers. Redrawn from \citeauthor{potti:11}.} \label{fig:FlyCatcher_genetic_corr}   %\cite{potti:11} 
\end{figure}
...
\ec{the individual has these genotypes, i.e. the probability they receive an additional allele $1$ or an allele $2$ from their other parent}:
\begin{equation} 
...
in each genotype \ec{and ignoring any non-additive effects of genotype}. \erin{I removed this sentence, replacing it with the phrase in blue above: These are the genotypic values of the trait that would result
...
our regression of phenotype on genotype ($\alpha_{\ell}$) \erin{ I changed phrasing throughout this section from regression 'of genotype on phenotype' to 'of phenotype on genotype' } does not depend on the population
...
from the other unrelated parent (the father). \ec{Therefore, the average
...
\ec{if there is dominance} our value of $V_A$ will not be constant across generations.
...
alleles by considering the effect of the alleles \ec{averaging over their possible genetic backgrounds (including the other interacting alleles they are possibly paired with), just as before.} We can then
...
twins better suited for this? \erin{I think this would be a better problem if we gave them numbers for covariances and asked them to estimate V dominance in the first part of this problem. }
\end{question}
