...
\end{marginfigure}
Children resemble their (biological) parents, that's because children inherit their genomes from
...
arrive at this result if we thought of the mother transmitting her own
maternal allele. By this
argument, $Cov(X_1,X_2) = \half Var(X_{M1}) + \half Var(X_{P1}) = \half
V_A$. \\  %The other half of the time she transmits her maternal allele to the child, in which case
...
\caption{Parent-midpoint offspring regression for beak depth and
  ...
  slope, whose slope is reported on the graph. Note that this The grey line is the
...
This way of estimating heritability has the problem of not controlling for
...
the heritability in the lab bur rather out in the real world and so we
don't want to  eliminate. In some organisms,
...
Despite its issues this measure of heritability is useful, it'll 
be directly relevant to our discussion of the response to selection in
...
there is potentially a tradeoff between the two aspects of males frog
calls. Indeed \citet{welch2014multivariate} found
