...
effect skin pigmentation often effect hair color), the genetic
linkage of loci independently affecting multiple traits, or the affects of assortative mating. 
...
=Cov(X_{1,A} ,X_{2,A} )$) between trait one and two. Thinking about this covariance arising due to pleiotropy, lets think about a set of $L$ SNPs contributing to our two traits. If the additive effect of an allele at the $i^{th}$ SNP is $\alpha_{i,1}$ and $\alpha_{i,2}$ on traits 1 and 2, then the additive covariance between our traits is
\begin{equation}
V_{A,1,2} = \sum_{i=1}^L 2\alpha_{i,1}\alpha_{i,2} p_i(1-p_i)
\end{equation}
assuming our loci are in linkage disequilibrium. Thus the genetic correlation arises due to pleiotropy, because loci that tend to affect trait 1 also systematically affect trait 2.  
...
\end{center}
\caption{The additive effect sizes of loci associated with Age at Menarche (AAM) on Age at Menarche and Height. } \label{fig:FlyCatcher_genetic_corr}   %\cite{potti:11} 
\end{marginfigure}
...
\end{equation}
we can generalize this to an arbitrary number of traits.
...
\end{equation}
where $V_{A,1}$ and $V_{A,2}$ is the additive genetic variance for trait 1 and 2 respectively. this is a statement to what extent the  additive genetic variance in two traits is correlated.   

One  type of  genetic covariance we often think about is the is covariance of male and female phenotypes. For example below is the relationship between the fore-head patch size for Pied fly-catchers fathers and their sons and daughters. The phenotype has been standardized to have variance $1$. The phenotypic covariance of the sample of fathers and sons is $0.35$, while the phenotypic covariance of fathers and daughter is $0.23$. 
...
{\bf A)} What is the additive genetic covariance between male-female patch size?\\
{\bf B)} What is the additive genetic correlation of male and female path size? You can assume that the additive genetic variance in the same in males and females.
\end{question}
