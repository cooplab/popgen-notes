...
{\bf B)} If we mixed the two populations together in equal proportions, \gc{i.e. average the frequency for $p_{AB}$ across both populations and so on}, what value would D take before any mating has had the chance to occur? \\
\end{question}
...
\end{center}
\caption{\gc{The decay of LD from an initial value of $D_0=0.25$ over time ($t$) due for a pair of loci $r$ apart.}} \label{fig:LD_time}
\end{marginfigure} 
\begin{marginfigure}
\begin{center}
\includegraphics[width=\textwidth]{figures/LD_decay/LD_decay_recom.pdf}
\end{center}
\caption{\gc{The decay of LD from an initial value of $D_0=0.25$ due to recombination, varying the recombination fraction ($r$) between our pair of loci for various times ($t$) that the LD has been decaying for.}} \label{fig:LD_recom}
\end{marginfigure} 
...
the \gc{Neanderthal and modern human populations} The two populations spread back into secondary contact when
