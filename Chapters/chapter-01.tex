...
\gc{
  ...
  \begin{marginfigure}
  \begin{center}
    \includegraphics[width= \textwidth]{figures/sharing_relatives/first_cousin_offspring.pdf}
\end{center}
\caption{.} \label{fig:kid_first_cousins}  
\end{marginfigure}
As we saw above close relatives are expected to share the IBD alleles in
large genomic blocks. Thus, when related individuals mate and transmit
alleles to an inbred offpspring, they transmit these alleles in big
blocks through meiosis. An example, lets return to the case of our
hypothetical first cousins from Figure
\ref{fig:IBD_cousins_chr_cartoon}, this pair of individuals had a
child and a possible pattern of transmission is shown in Figure
...
the granddparents. This region is homozygous by descent, an autozygous
segment, the child would homozygous at all of the loci in this
genome.

The inbreeding coefficient of the child sets the proportion of their
genome that will be these homozygous segments. For example, our child
of first full cousins is expected to have $\tidyfrac{1}{16}$ of their
genome in these segments.
The more distant the loop in the pedigree, the more meioses that
chromosomes have been through, the shorter these blocks will be. Our
...
have a high inbreeding coefficient, but have this distributed over
many small blocks of autozygosity. For example, Carlos the second had
...
genomic runs of homzygosity (ROH) sites in an individual's genome. One way to
...
As an example of this is from the work of
\citet{Sams:18}, who identified runs of homozygosity in 2,500 dogs,
ranging from 500kb up to many megabases. In Figure
\ref{fig:dog_FOH} is the distribution of $F_{ROH}$ of individuals in each dog
breed for the X and autosome. In Figure \ref{fig:dog_FOH_dist} this is
broken down by the length distibution. Dog breeds have been subject
to intense breeding that has resulted in high levels of inbreeding. 
Doberman Pinscher have the highest levels of their genome in runs of
homozygosity, however, much of this is due to many short blocks of
homozyosity. This indicates that the inbreeding in Doberman Pinscher
is old. While English bulldogs have lower levels of inbreeding,
however, this is contained in long blocks of homozygosity. This
suggests that English bulldogs have had high levels of inbreeding in
the recent past. 
...
\end{figure}
  ...
  \begin{figure}
  \begin{center}
    \includegraphics[width= \textwidth]{figures/sharing_relatives/dogs_FROH_dist.pdf}
\end{center}
\caption{Cumulative density of ROH from \citet{Sams:18} for various
  dog breeds.} \label{fig:dog_FOH}  
\end{figure}
...
$H_T$ with their averages across loci (as above). \sidenote{\ec{Averaging heterozygosity across loci first, then calculating $\fst$, rather than calculating $\fst$ for each locus individually and then taking the average, has better statistical properties as statistical noise in the denominator is averaged out.}}
