...
to be a per site (or per base) measure. \gc{For example, our ADH sequence covers $397$bp of DNA and so $\pi = 1.2\overline{6}/397=0.0032$ per site in {\it D. simulans} for this region. Note that we could also calculate $\pi$ per synonymous site (or non-synonymous), to do that we we would count up number of synonymous differences between our pairs of sequences, and then divide by the total number of sites where a synonymous change could have occurred.\sidenote{More technically we need to divide by the total number of one point mutations changes that would result in a synonymous change, this is because some mutational changes at a particular site will result in a N or S change depending on the base-pair change.}} 
...
individuals.  An example of using allele sharing to identify relatives is offered by
...
As an example of this consider the case of the Mexican wolf ({\it Canis lupus baileyi}), also known as the lobo, a sub-species of gray wolf. They were extirpated in the wild during the mid-1900s, and the remaining five lobos in the wild were captured to start a breeding program. \citet{vonHoldt:11} estimated the average expected heterozygosity to be $0.18$, based on allele frequencies at over forty thousand SNPs. However, the average lobos individual was only observed to  heterozygous at $12\%$ of these SNP. Therefore, the average inbreeding coefficient for the lobos is $F = 1 -\nicefrac{0.12}{0.18}$, i.e. $\sim 33 \%$ of their genomes are homozygous due to recent inbreeding. 
...
\erin{I might just add a footnote that it's improper to just avg. Fst across loci, citing Bhatia .. not for the undergraduates so much as the random graduate students who read your open notes as a reference}
...
and Lawrence's warbler (4 \& 3 in \ref{fig:blue_golden_warblers}), are actually found to just be hybirds \erin{is this pun on purpose? amazing!} between theses two species. 
...
{ \bf C)} Calculate mean $F_{ST}$ across both sub-populations. \erin{is this what you meant?}
\end{question}
