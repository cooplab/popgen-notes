...
\csvautobooktabular{Journal_figs/alleles_genotypes/ADH_MK/ADH.csv}  % can control this more https://mirror.hmc.edu/ctan/macros/latex/contrib/csvsimple/csvsimple.pdf
  \caption{Variable sites in exons 2 and 3 of the ADH gene in {\emph Drosophila} \citet{mcdonald:91}.  
...
(a-l) are from {\emph D. melanogaster};
    the next columns (a-f) give sequences from {\emph D. simulans}, the final 
  coumns (a-l ) from {\emph D. yakuba}.}  % I've dropped the
                                % heterozygote sites from D. yakuba.
  \label{Table:ADH}
\end{table}

\begin{question}
How many segregating sites are there in {\emph D. yakuba}?
What is the per base pair divergence are there between {\emph D. melanogaster} and {\emph D. yakuba}?
...
bears arent hybrids with polar bears, nor are they albinos, they are
homozygotes for a recessive change at the MC1R gene (with $GG$
individuals being white and $AA$ and $AG$ individuals being black.   
...
sample from British Columbi island populations \cite{RITLAND:01}. 
\begin{center}
\begin{tabular}{ccc}
\hline
$AA$ & $AG$ & $GG$ \\
...
\newthought{Individuals rarely mate complete at random}, therefore, genetic variation is never uniformly distributed across individuals.
