...
plot.lm.genos(a=a,p=0.1)
plot.lm.genos(a=a,p=p.eq)
plot.lm.genos(a=a,p=0.9)
...
plot.lm.genos<-function(a,p){
	...
	plot(y=range(a)*c(.5,1.5),x=c(-0.5,2.5),type="n",axes=FALSE)
	box()
	axis(2)
	axis(1,at=c(0,1,2))
	symbols(x=0:2,y=a,circles=3*sqrt(HWE)/(2*pi),bg=adjustcolor("blue",0.2),add=TRUE,inches=FALSE)  #ylim=range(a)*c(.2,1.8),xlim=c(-1,3)
