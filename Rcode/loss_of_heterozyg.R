png(file="~/Dropbox/Courses/PBGG_Core/Popgen_teaching_Notes/Figs/Loss_of_het.png",width = 800, height = 400)
N<-5
num.gens<-15
offset<-0.1
...
for(i in 1:num.gens){
 ...
 for(ind in 1:N){
		...
		lines(c(i,i+1), c(par[1]+which.allele*offset,ind-offset),col="light grey",lwd=0.5)
		new.cols[2*ind-1]<- my.cols[2*par[1] +ifelse(which.allele==1,0,-1)]

		which.allele<-sample(c(-1,1),1)
		lines(c(i,i+1), c(par[2]+which.allele*offset,ind+offset),col="light grey",lwd=0.5)
