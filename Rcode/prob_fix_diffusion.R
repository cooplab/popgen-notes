...
legend(x="topleft", legend=c(expression(N[e] == 2000),expression(N == 5000),expression(N == 10000),expression(N==infinity ~p[F]==s)),col=c("red","black","blue","black"),lwd=3,lty=c(rep(1,3),3))
...
plot(x=range(N),c(1e-5,1e5),log="xy",ylab=expression(2N log[10](p[F]( 1/ (2*N))))) ##" Ratio of Prob. of fixation "," to Neutral Prob.")),xlab=expression(paste(log[10](N[e])) ),type="n",axes=FALSE) #/(1/2*N[e])))
