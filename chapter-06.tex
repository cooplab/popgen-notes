...
\end{center}
\caption{} \label{fig:prob_fix_diffusion}
\end{figure}
...
and $N$ in Figure \ref{fig:prob_fix_diffusion}. \\

In the case where $Ns$ close to $1$ then
\begin{equation}
\pi(\frac{1}{2N}) \approx \frac{s}{1-e^{-2Ns}} \label{eqn:escape_from_intro}
\end{equation}
this is greater than our result $p_F=s$ from the branching process argument, increasingly so for smaller $N$. 
...
absorbed by the zero state. When the population size is finite, to fix we only need to reach a size 2N individuals. Weakly beneficial mutations (Ns~1) are slightly more likely to fix than the s probability, as they only have to reach 2N to never be lost.
...
\begin{equation}
\pi(\frac{1}{2N}) \approx \frac{s}{e^{2Ns}-1} \label{eqn:fix_deleterious}
\end{equation}
