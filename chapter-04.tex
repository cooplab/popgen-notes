...
To illustrate the approach we'll plot two different cases of dominance relationship in Figure \label{fig:add_dom} in the first row we show the
...
\end{equation}
Note that the slope of our regression of genotype on phenotype ($\alpha_{\ell}$) does not depend on
allele frequency for our completely additive locus (top row of
\ref{fig:add_dom}), changing the frequency at our locus does change
it's contribution to additive variance but only via the heterozygosity
($2p_{\ell}q_{\ell}$). In contrast, when there is dominance, our the
