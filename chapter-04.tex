...
along a single linear line (the red line in our Figure). The additive values are falling
...
population (which genotypes this is depends on the allele frequency). \\

The difference in the additive effect $a_{\ell, 2}-a_{\ell, 1}$ can be interpreted as a average effect of
...
slope of genotype on phenotype, bottom left Figure \ref{fig:add_dom},
as replacing a single copy of the 2 allele with a 1 allele in an
individual has a big
effect on average, as it will most likely move an individual from
...
$V_A$ and the additive effect of alleles.
...
\end{tabular}
\end{center}
\caption{Phenotypic covariance between some pairs of relatives. $^{*}$Assuming this is the only relationship
