...
and we will be more precise about what we mean by `past few generations' later on. For example, parent and child share exactly
one allele identical by descent at a locus, assuming that the two
parents of the child are randomly mated individuals from the
population. In figure \ref{fig:IBD_cousins_cartoon} I show a pedigree
demonstrating some configurations of IBD. \\
...
\caption{Alleles being transmitted through an inbred pedigree. The mum and the
  ...
  allele IBD. The offspring of first cousins being homozygous by
  descent at a locus.} \label{fig:IBD_cousins_cartoon}  
\end{figure}
...
population structure.  In figure \ref{fig:IBD_cousins_cartoon} our
...
\end{center}
\caption{A diagram showing the heirachical nature of F statistics. We
...
to that found in the total population ($H_T=2p_Tq_T$). } \label{fig:IBD_cousins_chr_cartoon}  
\end{figure}
